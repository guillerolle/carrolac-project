...
xhost +local:root &&\
docker run --rm -it --net=host --privileged --env="DISPLAY" --env="QT_X11_NO_MITSHM=1" --volume="/tmp/.X11-unix:/tmp/.X11-unix:rw" --volume="`pwd`/../docker-home:/home/duser:rw" --volume="/etc/localtime:/etc/localtime:ro" --volume="docker-site:/opt/ros/humble/lib/python3.10/site-packages" --volume="docker-dist:/opt/ros/humble/local/lib/python3.10/dist-packages" ros2:humble_with_gazebo &&\
xhost -local:root
